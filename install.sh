...
cp "$SCRIPT_DIR/launchers/lib.sh" "$LAUNCHER_DIR/"
chmod +x "$LAUNCHER_DIR"/*.sh
chmod +x "$LAUNCHER_DIR"/lib.sh

# Write install-time config so launchers can log to repo
cat >"$LAUNCHER_DIR/clocky.conf" << EOF
# Auto-generated by clocky-cli install.sh
CLOCKY_REPO_DIR="$SCRIPT_DIR"
EOF
...
echo "  clocky start <project> — start timer on a project"
